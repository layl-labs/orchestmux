...
워커가 보고를 마쳐도 pane은 **일부러 열어둔다.** 스크롤백이 그 결론에 *어떻게*
도달했는지 남은 유일한 기록이고, 보고는 틀릴 수 있다 — `done` 시점에 pane을 닫으면
그 보고를 검증할 증거가 사라진다. 결과를 다 읽은 뒤에 한 번에 치운다:

```bash
orchestmux sweep --dry-run   # 무엇이 지워질지 미리보기
orchestmux sweep             # 아직 작업 중인 워커는 남는다
```

보고 전에 pane이 죽은 워커는 해당 태스크도 `failed`로 표시해서, 아무것도
`dispatched` 상태로 남아 진행 중인 척하지 않게 한다.
