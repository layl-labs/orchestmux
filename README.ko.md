...
**tmux에서 여러 코딩 에이전트를 동시에 부리는 오케스트레이터입니다.**
...
에이전트를 여러 개 띄우는 것은 어렵지 않습니다. 정말 어려운 것은 **언제 끝났는지
아는 일**입니다. 터미널 출력을 눈으로 훑으며 "이제 끝났나?" 추측하는 순간부터
자동화는 무너집니다.

orchestmux는 그 빠진 조각을 채웁니다. 배정되는 모든 작업에 **보고 프로토콜**이
함께 주입되기 때문에, 코디네이터는 추측하는 대신 **진짜 완료 신호에 블로킹**할 수
있습니다.
...
| `send --to <w> --body "<텍스트>"` | 워커에게 메시지를 보냅니다 |
